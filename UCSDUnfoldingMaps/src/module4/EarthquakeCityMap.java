...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes () {
		...
		for (Marker m : countryMarkers) {
			...
			String cName = (String) m.getProperty ("name");
			for (Marker qm : quakeMarkers) {
				...
				if (em.isOnLand ()) {
					String qName = (String) em.getProperty ("country");
			...
			if (quakeCounter > 0) {
				oceanCounter -= quakeCounter;
				System.out.println ("The country " + cName + " has " + quakeCounter + " quakes");
			}
		}
		System.out.println ("The number of ocean quakes are " + oceanCounter);
